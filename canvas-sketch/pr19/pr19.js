...
class Particle {
  ...
  findCirclesPoints() {
    for (
      let xdiff = -this.rx / info.circleNum;
      xdiff <= this.rx;
      xdiff += this.rx / (info.circleIncrementalFactor * info.circleNum)
    ) {
      const x = this.x + xdiff;
      const xx = this.x - xdiff;
      const dd = 1 - (x - this.x) ** 2 / this.rx ** 2;
      const dy =
        this.ry * Math.sqrt(dd >= 0 ? dd : info.respectABS ? Math.abs(dd) : 0);
      const y = this.y + dy;
      const yy = this.y - dy;
      this.circleP.push(
        { x, y, incrX: -1, incrY: false },
        { x, y: yy, incrX: -1, incrY: false },
        { x: xx, y, incrX: -1, incrY: false },
        { x: xx, y: yy, incrX: -1, incrY: false }
      );
    }
  }
  ...
  updateCirclePoints() {
    this.circleP.forEach((co) => {
      ...
      co.x +=
        co.incrX *
        (info.randomUpdateXFactor > 1
          ? info.randomUpdateXFactor * Math.random()
          : 1);
  ...
  drawCircle(context, height) {
    this.circleP.forEach((co) => {
      ...
      const r =
        5 + (60 / height) * this.y - (60 / height ** 2) * this.y ** 2 + rgap;
      context.beginPath();
