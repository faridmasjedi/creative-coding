...
const randomRGB = () => `rgba(${randomNumber(255)}, ${randomNumber(255)}, ${randomNumber(255)}, ${randomNumber(1)})`;
...
class Grid {
  ...
  findPointPositions(width, height) {
    ...
    for (let i = 0; i < this.count; i++) {
      const x = ((i % this.colNum) + 1) * this.colGap + (width * (1-info.marginC) / 2);
      const y = Math.floor(i / this.colNum + 1) * this.rowGap + ( height * (1-info.marginC) / 2);
  ...
  updatePos(frame){
    ...
    for(let index in pointsObj){
      const noise = random.noise2D(pointsObj[index].x + frame, pointsObj[index].y,info.fr, 1)
