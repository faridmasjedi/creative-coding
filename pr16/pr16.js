...
const sketch = ({width, height, canvas}) => {
  ...
  for(let r = circleRaduis; r <= maxRaduis; r+= circleRaduis){
    const circleNum = Math.floor(r/firstRaduis)
    const theta = 2 * Math.PI / circleNum
    for (let t = 0; t < 2 * Math.PI; t+= theta) {
      const x = r * Math.cos(t)
      const y = r * Math.sin(t)
      const p = new Particle(x,y, width /2, height/2, (firstRaduis - 1) - 2 * r / reducedRaduisFactor)
      particles.push(p)
    }
  }
